...
const Filter = (props: { className?: string, useModal?:boolean }) => {
    const { updateData, clearPopup, selectsOption, filterOptions, setFilterOptions, isFilterDivVisible, setIsFilterDivVisible, filterDiv } = useContext(HeatMapContext)
    ...
    const filterTemplate =() => {
        const template = isFilterDivVisible && <div className='md:w-80  md:text-sm text-xs max-h-[80%] pointer-events-none   overflow-visible '>
            <div className="flex ">
                ...
                {
                    !props.useModal && <div className="bg-[var(--secondary-color)] w-fit h-fit rounded-lg p-2 pointer-events-auto" onClick={() => setIsFilterDivVisible((oldvalue: boolean) => !oldvalue)}>
    ...
    return (
        <div className={props.className || ""} ref={filterDiv} >

            {filterTemplate()}
         
            {
                (!isFilterDivVisible || props.useModal) && < div className="bg-[var(--secondary-color)] w-fit flex h-fit rounded-lg p-2 pointer-events-auto" onClick={() => setIsFilterDivVisible((oldvalue: boolean) => !oldvalue)}>
